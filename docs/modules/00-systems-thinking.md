...
因为今天走进小红书、X 或其他 AI Infra 社区，很容易看到：
...
这些内容并非没有价值。成熟系统的模块和源码值得学习，课程后面也会在合适的位置观察它们。

真正需要警惕的是另一件事：

> **不要把成熟系统最终长成的模块顺序，当成理解系统的起点。**
...
这条路径直接给出了答案，却隐藏了答案原本在解决什么问题。

这并不是 AI Infra 才有的现象。十年前，很多 Java 和 Spring Boot 培训也是从 ApplicationContext、Bean 生命周期、AOP 和自动配置一路钻进源码调用链。学完以后，你可以背出每个类在什么时候执行，却未必知道依赖管理、生命周期控制和横切逻辑分别是被什么问题逼出来的。

问题不在于读源码，而在于源码呈现的是成熟系统最终长成的答案。如果问题还没有出现，答案就很容易变成八股文。

今天把 Spring Boot 换成 vLLM，同一种学习顺序又回来了。

所以，区别不在于拆不拆 vLLM，而在于顺序。课程更希望这样学习：
...
到这里，我们其实已经有了两条不断放大的故事线。

一条从一个用户走向五万个用户，另一条从一张 GPU 走向上千张 GPU：
...
这让我想起 xkcd *What If?* 里的一个经典问题：如果地球上的所有人同时拿激光笔照向月球，能够让月球变色吗？<sup>[[2]](#ref-xkcd-laser)</sup>

普通激光笔当然不行。于是 Randall Munroe 不断提高功率。给每个人一套 Luxor 探照灯，再用透镜把光束聚焦到月球以后，月球暗面终于真的被照亮了——原问题到这里已经得到肯定答案。

但数字没有停在这里。功率继续增加，这个几毫瓦的玩具问题最终被放大成涉及行星级能源和灾难性物理后果的问题。
...
这里真正有意思的不是最后发生了什么，而是：

> **规模增加以后，问题并不只是原来的问题乘上 N。**

一支激光笔的时候，你不会考虑地球能源系统。功率继续扩大，能源、散热、大气和材料会依次进入问题，最后连月球本身都成了系统的一部分。

ML Systems 也沿着同样的路径展开。

### 从一个进程到许多台机器：机器之间也成了问题

推理这条线上，一个用户变成五个用户时，batching、排队和请求调度出现了；五个用户变成五百个、五万个用户时，单个进程已经不够，副本、负载均衡、网络和故障开始决定服务是否可用。

训练这条线上，一张 GPU 时，我们首先问：

> 模型放得下吗？

到了八张 GPU，问题变成怎么切分计算；到了上百张，还要追问怎么通信、谁在等待谁；到了上千张，拓扑、存储、调度和故障同样进入了系统边界。

这时我们需要的已经不只是“更多 GPU”，而是一整条相互配合的硬件路径：
...
许多张卡堆在一起，不会自动变成一台更大的计算机；许多份服务进程摆在一起，也不会自动变成一个可靠的线上服务。它们只有在拓扑、通信、调度和软件栈共同工作时，才可能成为一个可以使用的系统。

### 从许多张卡到数据中心：物理世界出现了

再把数字往上加，问题会继续越过服务器机箱。

> 电从哪里来？热怎么出去？网络怎么铺？光模块够不够？数据中心建在哪里？

GPU 不是插在空气里运行的。在一块加速卡进入集群之前，需要先有土地、园区、建筑、供电接入、变电设施、备用电源、冷却系统、消防系统和物理安全。

于是有一天你抬头发现：我们一开始明明只是写了：

```python
model.generate(...)
```

为什么现在居然开始讨论“水电还是核电”了？

这并不是为了制造戏剧效果。2025 年 1 月，SoftBank 与 OpenAI 宣布牵头成立美国人工智能基础设施项目“星际之门”（Stargate Project），Oracle 和 MGX 也是初始出资方。项目最初宣布计划在四年内投入最高 5000 亿美元，并立即开始部署其中 1000 亿美元。SoftBank 主要承担财务责任，OpenAI 主要承担运营责任。<sup>[[4]](#ref-stargate-announcement)</sup>

这里的“5000 亿美元”，不能简单理解成 OpenAI 准备下单购买同等金额的 GPU。它指向的是一条从资本和物理建设一直延伸到可用 AI 能力的基础设施链。
...
当项目规模以 GW，也就是十亿瓦为单位讨论时，电力已经不再是服务器规格表最后一行的 TDP，而变成：
...
如此巨大的基础设施不可能由一家模型公司独自完成。这里面需要有人融资，有人承担资本开支，有人建设数据中心，有人提供云和数据库基础设施，有人提供芯片、网络和软件栈，还有人负责把这些资源最终变成可以训练和服务模型的计算系统。
...
Stargate 的官方叙事也不只把它当作技术项目。它同时讨论美国的 AI 领导地位、就业、再工业化、供应链和国家安全。当数字扩大到这里，最初的 Python 脚本已经穿过机器、集群和数据中心，进入工业与国家战略的尺度。<sup>[[4]](#ref-stargate-announcement)</sup>

### 但把机器堆起来，还没有得到能力

这条放大链很容易诱导出一种过于简单的线性想象：
...
问题是，有了楼、电和机器，我们仍然只拥有资源。

训练 workload 应该放到哪些机器？模型如何并行？失败以后如何恢复？数据如何持续供给？推理请求如何调度？训练、实验和线上服务又该怎样争夺同一批资源？

**从“拥有算力”到“获得模型能力”之间，仍然隔着一个完整的 ML system。**

所以，当 Stargate 把“还能增加多少机器”推到极致时，以 DeepSeek-V3 为代表的国产模型与系统工作也在追问另一半问题：同样的机器，能不能完成更多有效工作？

DeepSeek-V3 的技术报告强调了 MoE、FP8 训练、计算与通信重叠等设计，并报告完整训练使用约 278.8 万 H800 GPU 小时。这个数字当然一点也不“小”，但它说明模型能力并不只由卡的数量决定，也取决于模型结构、数值精度、并行策略、通信设计、负载均衡、训练框架，以及算法和硬件能否共同设计。<sup>[[6]](#ref-deepseek-v3)</sup>

因此，更准确的评价不是“效率证明基础设施不重要”，而是：

> **基础设施规模决定资源上限，系统效率决定我们离这个上限还有多远。**

效率提升和基础设施扩张并不是相反方向。如果一种系统设计能让同样的机器完成更多工作，那么一座大型数据中心就可以训练更多模型、服务更多用户、尝试更多实验。两者相乘，才是最终能够获得的能力。
...
每把数字往上加一次，新的约束就会进入系统边界；而每进入一个新的尺度，我们又必须追问，如何把新增的资源真正转化成有效工作。
...
如果学完这门课以后，你只是多背了几十道 AI Infra 八股，
