...
```text
代码 -> 发现系统问题 -> 建立可信测量 -> 理解 workload
     -> 单卡执行 -> 分布式训练 -> 集群任务 -> 生产服务
     -> 持续运维 -> 系统设计
```
...
当前第 0 章和规模效应插入章为 **初稿**，其余 16 章为 **占位**。状态只表示内容成熟度，不代表课程大纲已经冻结。

## Part 0 — Systems Thinking for ML

| 章 | 事件 / 问题 | 关键产出 | 状态 |
| --- | --- | --- | --- |
| 0 | [为啥需要 MLSys？](/modules/00-systems-thinking) | 课程问题、系统视角与调查循环 | 初稿 |
| X | [规模效应：从单卡到多卡，从单机到多机](/modules/0X-scaling) | 规模、扩展效率与成本的数量级直觉 | 初稿 |
